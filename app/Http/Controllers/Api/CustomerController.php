...
class CustomerController extends Controller
{
    ...
    public function user_issues(Request $request)
    {
        ...
        if($request->status == 'All'){
            $issues = Issue::where('flat_id', $flat->id)->with(['department','photos','comments.replies.user'])->get();
        }else{
            $issues = Issue::where('flat_id', $flat->id)->where('status',$request->status)->with(['department','photos','comments.replies.user'])->get();
        }
        return response()->json([
                'issues' => $issues
        ],200);
    ...
    public function maintenance_payments(Request $request)
    {
        ...
        //     ->orderBy('id', 'desc')
        //     ->get();
        $transactions = Transaction::where('user_id',$user->id)->where('model','Maintenance')->with(['maintenance_payments.user','maintenance_payments.flat.block.building',])->orderBy('id', 'desc')->get();
    ...
    public function create_maintenance_payment_order(Request $request)
    {
        ...
        $orderData = [
            ...
            'amount'          => $item_amount, // 2000 rupees in paise
            'currency'        => 'INR',
    ...
    public function get_visitors(Request $request)
    {
        ...
        $user = Auth::User();
        if($request->type == 'All'){
            $visitors = $user->visitors()->where('status','!=','Completed')->with(['gate_passes','vehicles','inouts'])->get();
        }else if($request->type == 'Completed'){
            $visitors = $user->visitors()->where('status',$request->type)->with(['gate_passes','vehicles','inouts'])->get();
        }else{
            $visitors = $user->visitors()->where('type',$request->type)->where('status','!=','Completed')->with(['gate_passes','vehicles','inouts'])->get();
        }
        return response()->json([
            'visitors' => $visitors
        ], 200);
    ...
    public function get_issues(Request $request)
    {
        ...
        ->with(['flat.tanent','flat.owner','block','building','photos','department','comments.user','comments.replies.user'])->get();
        return response()->json([
                'issues' => $issues
        ],200);
    }
