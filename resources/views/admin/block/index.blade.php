              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td>{{$block->name}}</td>
                    <td>{{$block->flats->count()}}</td>
                    <td>{{$block->gates->count()}}</td>
                    <td>{{$block->guards->count()}}</td>
                    <td>{{$block->status}}</td>
                    <td>
                      <a href="{{route('block.show',$block->id)}}" target="_blank"  class="btn btn-sm btn-warning"><i class="fa fa-eye"></i></a>
                      <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addModal" data-id="{{$block->id}}" data-name="{{$block->name}}" data-status="{{$block->status}}">
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{route('block.store')}}" method="post" class="add-form" enctype="multipart/form-data">
        @csrf
